  ...
  <div class="bg-gradient-to-b from-green-100 via-[#CBEDD5] to-green-600/30">
    <div class="container flex flex-col items-center pt-24 px-4">
      <TransitionRoot appear :show="isVisible" as="template">
        <div class="max-w-4xl mx-auto text-center space-y-10">
          ...
          <div class="space-y-4">
            ...
            <!-- Subtitle -->
            <TransitionChild as="template" enter="transform transition duration-1000 delay-300" enter-from="opacity-0"
              enter-to="opacity-100" leave="transform duration-300 transition ease-in-out" leave-from="opacity-100"
              leave-to="opacity-0">
          ...
          <!-- Buttons -->
          <TransitionChild as="template" enter="transform transition duration-1000 delay-500"
            enter-from="opacity-0 translate-y-4" enter-to="opacity-100 translate-y-0"
            leave="transform duration-300 transition ease-in-out" leave-from="opacity-100 translate-y-0"
            leave-to="opacity-0 translate-y-4">
            <div class="flex flex-col items-center sm:flex-row gap-4 justify-center">
              <a href="#menu"
                class="px-8 py-3 md:px-4 md:py-2 bg-emerald-600 text-white font-medium text-lg md:text-base md:font-normal tracking-wider rounded-lg hover:bg-emerald-700  transition-colors duration-300 shadow-lg hover:shadow-xl">
                Mulai Sekarang
              </a>
            </div>
          </TransitionChild>
      ...
      <TransitionRoot appear :show="isVisible" as="div">
        <div class="space-y-10 text-center mt-6">
          <TransitionChild as="template" enter="transform transition duration-1000 delay-700"
            enter-from="opacity-0 translate-y-4" enter-to="opacity-100 translate-y-0"
            leave="transform duration-300 transition ease-in-out" leave-from="opacity-100 translate-y-0"
            leave-to="opacity-0 translate-y-4">
          ...
          <TransitionChild as="template" enter="transform transition duration-1000 delay-1000"
            ...
            leave-to="opacity-0 translate-y-4">
            <div class="my-6">
              <span class="text-4xl md:text-2xl font-semibold text-green-800">Cari Kebutuhanmu</span>
            </div>
          </TransitionChild>
          <TransitionChild as="template" enter="transform transition duration-1000 delay-1200"
            enter-from="opacity-0 translate-y-4" enter-to="opacity-100 translate-y-0"
            leave="transform duration-300 transition ease-in-out" leave-from="opacity-100 translate-y-0"
            leave-to="opacity-0 translate-y-4">
...
const { data, isLoading, refetch } = useQuery({
  ...
  queryFn: async () => await axios.get('https://quran-api-id.vercel.app/random').then((res: any) => res.data)
})
